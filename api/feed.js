...
export default async function handler(req, res) {
  ...
  const isCronCall = Boolean(CRON_SECRET) && secret === CRON_SECRET;

  // === Публичное чтение ленты (фронтенд) ===
  // Обычный GET без параметра refresh просто отдаёт сохранённые твиты.
  if (req.method === 'GET' && !req.query.refresh) {
    const feed = await readFeed();
    return res.status(200).json({ tweets: feed });
  }

  // === Всё, что ниже, — обновление ленты. Требует секрет. ===
  if (!isCronCall) {
    return res.status(401).json({ error: 'unauthorized' });
  }

  if (!API_KEY) {
    console.error('TWITTERAPI_KEY is not set');
