...
// Берём транзакции (SOL transfer activity) кошелька, фильтруем по времени
async function getActivities(address, apiKey, sinceUnix) {
  let page = 1;
  const pageSize = 40;
  let all = [];
  // SolScan paginates; ограничим по разумному кол-ву страниц, чтобы не упереться в лимиты
  for (let i = 0; i < 10; i++) {
